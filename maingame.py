...
# Load images
aard = pygame.image.load("aard.png")
axii = pygame.image.load("axii.png")
igni = pygame.image.load("igni.png")
tile = pygame.image.load("tile.png")

# Adding images to a list
imagelist.append(aard)
imagelist.append(axii)
imagelist.append(igni)
...
def start_game():
    ...
    pygame.display.set_caption('Sam tiles Game') #Title
    clock = pygame.time.Clock() # Track time
    bg = pygame.Surface((WIDTH,HEIGHT))
    SCREEN.fill(WHITE)
    display_text('Sam Tile Game',600,250,100)
    display_text('Start',350,500,80)
    display_text('Quit Game',850,500,80)    
    pygame.display.flip()
    running = True
    while(running):
        for event in pygame.event.get(): # QUIT
